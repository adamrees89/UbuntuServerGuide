...
\subsection{Apt and apt-get} 
\label{ssec:apt}

Apt is short for \textit{Advanced Package Tool}, it is a command line program for installing, removing and managing software on your system.  It is a front end to the dpkg package manager, and is used to install packages from the repositories. Generally, apt is used for interactive use, and apt-get is used for scripting.  The syntax is simple, to install a package simply type;
...
One general maintenance task is to update and upgrade the system, this is done by running the following two commands;
...
Which can be run in one line using;

\begin{lstlisting}
sudo apt update && sudo apt upgrade -y 
\end{lstlisting}

Apt command line options are shown in table~\ref{tab:apt}.
...
--only-upgrade & Only upgrade the package, do not install if not already installed\\
--allow-downgrades & Allow downgrading of packages\\
--allow-remove-essential & Allow removing of essential packages\\
--allow-change-held-packages & Allow changes to held packages\\
\hline
\end{tabular}
\caption{Table of apt options}
\label{tab:apt}
\end{table}
...
To shutdown or reboot the system from the command line simply type;

\begin{lstlisting}
sudo shutdown -h now
\end{lstlisting}

to shutdown immediately, or

\begin{lstlisting}
sudo shutdown -r now
\end{lstlisting}

to reboot immediately.  You can also specify a time to shutdown or reboot, for example to shutdown in 30 minutes type;

\begin{lstlisting}
sudo shutdown -h +30
\end{lstlisting}

Command line options for shutdown are shown in table~\ref{tab:shutdown}.
...
-t & Set the time to shutdown or reboot (e.g. +30 for 30 minutes)\\
-c & Cancel a scheduled shutdown or reboot\\
\hline
\end{tabular}
\caption{Table of shutdown options}
\label{tab:shutdown}
\end{table}
...
\textbf{sl} is a bit of fun that shows a 'steam locomotive' every time you accidentally type \verb|sl| instead of \verb|ls|.  Try out the following;
...
Where the \textbar character is the pipe command.
