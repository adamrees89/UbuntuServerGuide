...
\todo[inline,color=blue!40]{Describe how to use git for version control of files on the server, and how to use it with github in this section.}

\subsection{Install Git}
\label{ssec:git_install}

To install git on your server simply run the following command;
...
To setup your linux server to work with github, you need to generate a ssh key pair on your server and then add the public key to your github account.  To generate the ssh key pair simply run the following command;

\begin{lstlisting}
ssh-keygen -t ed25519 -C "your.email@example.com"
\end{lstlisting}

This will generate a public and private key pair in the $/home/username/.ssh$ directory.  The public key is the one with the .pub extension, so in this case it would be $id\_rsa.pub$.  To add this to your github account simply copy the contents of the file and add it to your github account under settings, ssh keys.

Now we need to configure git on our server, we can do this by running the following commands;
...
This section will outline some of the basic methods for monitoring network traffic on your LAN. For monitoring network traffic on your server, please refer to Section~\ref{sec:monitoring}. This section is useful for checking if there are unusual devices on your home network.

\subsection{nmap}
\label{sec:nmap}

nmap, short for network mapper, is a powerful tool for scanning your network and finding out what devices are connected to it.  To install nmap simply run the following command;

\begin{lstlisting}
sudo apt-get install nmap
\end{lstlisting}

To scan your network for devices simply run the following command;

\begin{lstlisting}
sudo nmap -sn 192.168.1.0/24
\end{lstlisting}

This will give you a list of all the devices on your network, along with their IP addresses and MAC addresses.  You can also use nmap to scan for open ports on a specific device, simply run the following command;

\begin{lstlisting}
sudo nmap -p- IP_ADDRESS
\end{lstlisting}

nmap has a large number of options and can be used for a variety of purposes, I would recommend reading the nmap documentation for more information on how to use it effectively. A short table of some of the most useful options is shown in Table~\ref{tab:nmap}.

\textit{Note} - nmap is a powerful tool and can be used for malicious purposes, so please use it responsibly and only on networks that you own or have permission to scan.

\textit{Note} - the commands below must be run with root privileges, so you may need to add \textbf{sudo} to the beginning of the commands.
