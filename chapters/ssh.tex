...
This will generate the keys using a RSA authentication identity of the user. During the process you will be prompted for a password. Simply hit Enter when prompted to create the key.
...
is the private key. Now copy the $id\_rsa$ file to the client, then append $id\_rsa.pub$ to;
$$
...
However if the computer is a fresh installation, then you can simply change the name of the $id\_rsa.pub$ file to $authorized\_keys$.
...
\subsubsection{Disable X11 Forwarding}

X11 connections are a graphical interface that can be forwarded over ssh, servers typically don't need GUI's so if you don't need it then disable it. 
If you do need it, then make sure that you have a firewall in place to protect your server from the internet. 

\begin{verbatim}
X11Forwarding no
\end{verbatim}

\subsubsection{Disable TCP Forwarding}

TCP forwarding allows the server to forward TCP connections to other servers, which can be a security risk if the server is compromised.

\begin{verbatim}
AllowTcpForwarding no
\end{verbatim}

\subsubsection{Disable Agent Forwarding}

Agent forwarding allows the server to forward the authentication agent connection to other servers, which can be a security risk if the server is compromised.

\begin{verbatim}
AllowAgentForwarding no
\end{verbatim}

\subsubsection{Login Grace Time}

Login grace time is the amount of time that the server will wait for a user to successfully log in before disconnecting.  This is useful for preventing brute force attacks.

\begin{verbatim}
LoginGraceTime 30
\end{verbatim}

\subsubsection{Max Auth Tries}

Maximum authentication attempts is the number of times that a user can attempt to log in before being disconnected.  This is useful for preventing brute force attacks.
...
Just make sure that you constantly update your ssh server instance, use \textit{apt-get or apt} (See~\autoref{ssec:apt}).
